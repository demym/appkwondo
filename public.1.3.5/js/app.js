...
//var rooturl="http://9.71.213.75:3000";
...
function tabbize(divselector) {
	...
	$(divselector).swipeleft(function (event) {
		if (current + step < 0 || current + step > maximum - visible) { return; }
			...
			$(ulselector).animate({ left: -(liSize * current) }, speed, null);
		}
		return false;
	});
...
function getTkdTechnologyNew(tkdt_garaid, callback) {
	...
	$.ajax({
		...
		.done(function (data) {
			...
			//console.log(tkdtnew);
			var html = new EJS({ url: 'tpl/tkdtnew.ejs' }).render(tkdtnew);
	...
	if (tkdt_iscritti.rows.length > 0) {
		...
		$(tkdt_iscritti.rows).each(function (i) {
			...
			if ((k.trim().toLowerCase() == chiave.trim().toLowerCase()) || (k2.trim().toLowerCase() == chiave.trim().toLowerCase())) {
				...
				var html = new EJS({ url: 'tpl/mytabulati.ejs' }).render(tabuls);
			...
			getTkdTechnology(function (htm) {
				...
				if (tkdtcontent.trim() != "") {
					if (htm != tkdtcontent) {
						...
						snotify({ text: "Contenuto Tkdt variato" });
						tkdtcontent = htm;
...
function docready() {
	...
	*/
	...
	$(window).on("navigate", function (event, data) {
		...
		console.log("navigateevent", data);

		if ((data.state.pageUrl == "popResult") && (data.state.direction == "back")) {
			console.log("navigated back to console");
	...
	*/
	...
	socket.on('notification', function (data) {
		console.log("received notification from socket.io server:")
		console.log(data);
	...
	socket.on('refreshgara', function (data) {
		console.log("received refreshgara from socket.io server:")
		console.log(data);
...
function clickRealtime() {
	...
	//sendRealtime(true);
	console.log("realtimeindex before call: " + realtimeindex);
	$.post(rooturl + urln, doc, function () {

		console.log("realtime setted to " + active + " for matchid " + matchid);
	...
	$.extend(opt, options);
	   ...
	   $.ajax({
		...
		.done(function (data) {
			...
			var html = new EJS({ url: 'tpl/matchesbyprog2.ejs' }).render($matches.rows);
			//alert(html);
...
function renderMatchesByProg($mbp) {
	...
	var html = new EJS({ url: 'tpl/matchesbyprog2.ejs' }).render($mbp.rows);
	//alert(html);
	...
	colog("loadmatchesbyatleta " + id);
	   $.ajax({
		...
		.done(function (data) {
			...
			var html = new EJS({ url: 'tpl/matchesbyatleta4.ejs' }).render($matchesbyatleta);
...
function renderMatchesByAtleta($mba) {
	...
	conslog($mba);
			 ...
			 var html = new EJS({ url: 'tpl/matchesbyatleta4.ejs' }).render($mba);
	var listaname = "listabyatleta";
	var listview = $("ul#" + listaname);
	listview.empty();
	listview.append(html);
...
function getMpaHistoryAtleta() {
	...
	$.get(url, function (dat) {
		...
		var html = new EJS({ url: 'tpl/mpahistoryatleta.ejs' }).render(dat);
		//console.log(html);
...
function showMatchesForAtleta(id) {
	...
	var html = new EJS({ url: 'tpl/matchesforatleta2.ejs' }).render($matchesForAtleta);
...
function viewConsoleOld(idx) {
	...
	var html = new EJS({ url: 'tpl/matchconsole.ejs' }).render(idx);
...
function renderConsoleTabs() {
	...
	console.log("renderconsoletabs", consoles);
...
function checkConsoles(id, rta) {
	...
	console.log("checking if match " + id + " already in consoles array");
	var found = false;
	...
	if (!found) {
		console.log("match is not into consoles array, adding it");
		var newrta = {
			active: false,
		...
		console.log("match " + id + " added to consoles array");



	} else {
		console.log("match " + id + " already in consoles array, doing nothing");
	}
...
function setResult(obj) {
	...
	console.log("last entered match", lastenteredmatch);
	conslog(mtch);
	var rt = false;
	if (mtch.realtime) {
	...
	if (rt) {
		...
		console.log("you entered a realtime match");
		var idx = 0;
		rtdiv.show();
...
function sgetMatchesForAtleta(input, atletaid) {
	...
	var mfa = filterRows($m, { "atletaid": atletaid }, true);
	//console.log("mfa",mfa);
...
function setResultOK() {
	...
	var mfa = filterRows(jGara.matchesbyprog, { "atletaid": atl.id });
...
function test() {
	...
	//console.log(JSON.stringify($a));
		  ...
		  var html = new EJS({ url: 'tpl/matchesbyprog2.ejs' }).render($a.rows);
	var listaname = "listabyprog";
	var listview = $("ul#" + listaname);
	listview.empty();
	listview.append(html);
...
function listChats() {
	...
	$.ajax({
		...
		.done(function (data) {
			...
			progressStop();
			var html = new EJS({ url: 'tpl/chatlist.ejs' }).render(data.rows);
...
function gotoTempoReale() {
	...
	var html = new EJS({ url: 'tpl/realtimeconsole.ejs' }).render(realtimeArray);
...
function getCategorieCoperte() {
	...
	if (jGara) {
		if (jGara.gara) {
			...
			$roz = filterArray(tkdtiscritti, { societa: "A.S.D. TAEKWONDO ROZZANO" }, true);
			//console.log($roz);
