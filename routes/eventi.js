...
var fname = "eventi.xml";
var jfname = "eventi.json";
...
function readGareMongo(callback) {
	mongo.getfile(jfname, function (data) {
		...
		gare = data;
		console.log("read eventi from mongo " + jfname + " :   " + gare.rows.length);
		callback();
...
function readGareAltervista(callback) {
	...
	console.log("readEventiAltervista")
	var fname = "eventi.json";
	request.get(altervistadataurl + fname, function (error, response, body) {
		if (!error && response.statusCode == 200) {
			var data = JSON.parse(body);

			gare = data;
			console.log("read eventi from altervista " + fname + " :   " + gare.rows.length);
			callback(gare);
...
function readGareJson(callback) {
	if (usemongo) {
		...
		fs.readFile(fname, "utf-8", function (err, data) {
			...
			console.log("read eventi from file " + fname + " :   " + gare.rows.length);
			callback();
			//console.log(data);
		});
			...
			fs.writeFile(fname, xml, function (err) {
				...
				fs.readFile(fname, "utf-8", function (err, data) {
					...
					parser.parseString(data, function (err, result) {
						...
						gare = xform(gare);
						fs.writeFile('data/eventi.json', JSON.stringify(gare), function (err) {
							if (err) return console.log(err);
							console.log('File eventi.json saved on server');
							var dat = require('../data/eventi.json');
...
router.get('/findall', function (req, res) {
	...
	console.log("called eventi findall for societa " + societa);
...
router.get("/fullgarabyid/:id", function (req, res) {
	...
	mongo.getfile("atleti.json", function (atleti) {
		...
		findById(id, function (obj) {
			...
			console.log("got gara " + id + ": rows: " + obj.rows.length);
			ret.gara = obj;
...
router.post('/add', function (req, res) {
	...
	if (usemongo) {
		mongo.addRecord("eventi.json", id, row, function (data) {
...
router.post('/delete', function (req, res) {
	...
	if (usemongo) {
		mongo.deleteRecord("eventi.json", id, function (data) {
			console.log("deleted");
			//ressent=true;
			res.send(data);
...
router.post('/update', function (req, res) {
	...
	console.log("trying to update evento id " + id);


	if (usemongo) {
		mongo.updateRecord("eventi.json", id, row, function (data) {
			console.log("updated");
			//ressent=true;
			res.send(data);
...
function findById(id, callback) {
	...
	mongo.getfile("eventi.json", function (gare) {

		colog('findEventiById: ' + id);
		//console.log(JSON.stringify(gare));
