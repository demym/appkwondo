...
/*
	...
	*/
	...
	socket.on('refreshchat', function (data) {
		conslog("received socket refreshchat ");
		refreshChat();
		chatunreadcount = 0;
	...
	socket.on('getnickname', function (data) {
		conslog("getnickname: " + data.sockid);
	...
	socket.on('notification', function (data) {
		conslog("received notification from socket.io server:")
		conslog(data);
	...
	socket.on('refreshgara', function (data) {
		conslog("received refreshgara from socket.io server:", data)
	...
	socket.on('message', function (data) {
		colog('Received a message from the server!', data);
		...
		if (lid == mid) {
			...
			if (ev == "matchstart") {
				...
				if (data.active) {
					...
					var ris = data.result.split("-");
					if (ris.length > 0) {
						//console.log("ris", ris, data);
						/*
						if (parseInt(ris[0], 10) >= parseInt(ris[1], 10)) {
							playSound("img/puntoafavore");
							
						} else playSound("img/puntocontro");*/
						//playVoice(data.match.atletaname+", "+data.result.replace("-","a"));
						playVoice(data.match.atletaname + " " + data.result.replace("-", " a "));
					}
...
function playVoice(text) {
	...
	conslog("settings.voice", settings.voice);
