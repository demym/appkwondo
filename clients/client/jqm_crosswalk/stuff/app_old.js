...
var app = {
 ...
 function initApp() {
   ...
   //alerta("Running on phone: "+isPhone);
...
("ui-state-persist");
	...
	gara.loadById(id,function(data) {
	   ...
	   $("#gara #ligaralocation").html("Location: "+$gara.doc.location);
...
function doLogin(){
	...
	var ck=page.find("#chck-rememberme").attr("checked");
  ...
  $.ajax({
	...
	function createHtmListview(obj)
	{
     ...
     var pophtm=''; 	 
					   for (var key in obj) {
                         colog(' name=' + key + ' value=' + obj[key]);
                         pophtm+='<li><span class="small">'+key+'</span><br><span class="medium">'+obj[key]+'</li>';
...
function sysMsg()
	  ...
	  */
	  ...
	  $("#gara div[data-role=content]").append(html);
	  $("#gara #popSysMsg").popup();
	  $("#gara #popSysMsg").popup("open");
	 }
	 
	 
	 function sysMsgOk(){
		 //alert("sysmsgok")
		 var text=$("#gara #popSysMsg #tamsg").val();
  ...
  function setResult(obj)
	 ...
	 function getDerby(id){
		 colog("getDerby for matchid "+id);
		 var retvalue={ rows:[]};
		 var samemid={ rows:[]}
		 var $r=filterRows($allmatches,{ id: id});
			   ...
			   var $r1=filterRows($allmatches,{ matchid: mid, dadisputare: "yes", disputato: "no"})
			    colog("incontri con matchid="+mid+": "+$r1.rows.length);
			 $($r1.rows).each(function(i){
				 var row=$r1.rows[i];
				 var rid=row.doc.id;
				 if (rid!=id) {
					samemid.rows.push(row);
