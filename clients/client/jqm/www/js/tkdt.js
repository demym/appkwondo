...
function syncGaraWithTkdt(obj, callback) {
    ...
    var isForme = false;
    if (tipo.val().toLowerCase() == "forme") isForme = true;
    var gara = null;
    if (tid.trim() == "") {
    ...
    if (!isForme) {
        ...
        var url = rooturl + "/tkdt/retrieve/" + tid;
        ...
        var url = rooturl + "/tkdt/retrieveforme/" + tid;
        progressStart("Recupero dati ufficiali di gara " + tid + " ....")
        $.ajax({
                type: "GET",
                url: url
...
function getTkdtAtletiCategoriaForme(cateta, catcintura, cattipo, sesso) {



    console.log("getTkdtAtletiCategoria searching atleti in categoria", cateta + " " + catcintura + " " + cattipo + " " + sesso)
    var retvalue = "notfound";
    var avversari = [];
    if (!cattipo) return avversari;
    ...
    $(source).each(function (i) {
        ...
        var ctipo = "";
        if (atl.cattipo) ctipo = atl.cattipo.toLowerCase();
        //var ctipo = atl.cattipo.toLowerCase();
        var ccint = atl.catcintura.toLowerCase();
        var ceta = atl.cateta.toLowerCase();
        var csesso = atl.sesso.toLowerCase();
        ...
        var doIt = true;
        if (ctipo != cattipo.toLowerCase()) doIt = doIt && false;
        if (ccint != catcintura.toLowerCase()) doIt = doIt && false;
        if (ceta != cateta.toLowerCase()) doIt = doIt && false;
        if (csesso != sesso.toLowerCase()) doIt = doIt && false;
...
function matchTkdtIscritti(obj) {
    ...
    if (editgaratkdt.atleti_iscritti) json = editgaratkdt;
...
function getCategorieCoperte(societa) {
    ...
    if (!jGara.gara.rows[0].doc.tkdt) return result;
    if (jGara) {
        if (jGara.gara) {
...
function getTpss(html) {
    ...
    var html = $("#page_tpss textarea#html").val();
    //console.log(html);
    $("#page_tpss #tpsshtml").html(html);

    //return;

    atls = [];
    var table = $(document).find("#page_tpss table#Table1");
    table.attr("border", "1");
    ...
    $("#page_tpss textarea#html").val(JSON.stringify(json));
