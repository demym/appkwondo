...
function showTkdtSocieta() {
 ...
 $.each(socs, function (index, item) {
	...
	html+='<div data-role="collapsible" class="collaps" data-theme="a"><h3>'+item.societa+' ('+item.atleti.length+')</h3><ul data-role="listview">';
...
function refreshRealtime(mbyprog) {
	...
	console.log("realtime matches", realtimeArray.length);
...
*/
	...
	ace.load("native://Native2.xaml", function (page) {
...
function addToListBox(listBox) {
	// Add prime numbers
	var obj = new ace.NativeObject(ace.valueOn({
		android: "mypackage.MyAlgorithm",
		ios: "MyAlgorithm"
	}));
