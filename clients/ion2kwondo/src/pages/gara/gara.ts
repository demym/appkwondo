...
import { NavController, NavParams, ModalController, Navbar,LoadingController } from 'ionic-angular';
...
export class GaraPage {
  ...
  constructor(public loadingCtrl: LoadingController, public modalCtrl: ModalController, public deviceFeedback: DeviceFeedback, public events: Events, public backend: BackendProvider, public navCtrl: NavController, public navParams: NavParams) {
    var questo = this;
  ...
  selectIscritti(){
    var questo=this;
    let profileModal = this.modalCtrl.create('IscrittiPage', { gara: questo.gara });
    profileModal.onDidDismiss(data => {
      if (data){
        var iscritti=data.iscr;
        if (data.count==0) iscritti="";
