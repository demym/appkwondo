...
export class BackendProvider {
  ...
  constructor(public alertCtrl: AlertController, private uniqueDeviceID: UniqueDeviceID, private tts: TextToSpeech, private backgroundMode: BackgroundMode, private localNotifications: LocalNotifications, public badge: Badge, public feedback: DeviceFeedback, private storage: Storage, public events: Events, public platform: Platform, public http: Http, private utils: UtilsProvider) {
  ...
  setupNavbarBack(nb, nc) {
    if (1 == 1) return;
    var questo = this;

    nb.backButtonClick = (e: UIEvent) => {
      // todo something
...
*/
  ...
  isCordovaAndroid(){
    ...
    var retvalue=false;
    if (questo.isCordova()){
      if (questo.platform.is("android")) retvalue=true;
    }
    return retvalue;
  }
  ...
  getUniqueDeviceID(callback?: any) {
    var questo = this;
    if (questo.platform.is("cordova")) {
