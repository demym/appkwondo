...
function recordAudio() {
	 ...
	 if (recording) {
		 ...
		 $("#matchesatleta  #popResult a#recordaudio").text("RECORD").button("refresh")
		 $("#matchesatleta  #popResult a#recordaudio").css("background","red");
		 recorder.stop();
		 //recorder.playback();
	 }	 
	 if (!recording) {

		 $("#matchesatleta  #popResult a#recordaudio").text("RECORDING").button("refresh")
		 $("#matchesatleta  #popResult a#recordaudio").css("background","silver");
		 recorder.record();  
...
function sendRealtime() {
	...
	var rtext="Round "+realtime.round;
	if (realtime.round=="GP") rtext="GoldenPoint"
    
	text+=" "+rtext;
	
	var mtext="in pausa";
	if (realtime.running) mtext="IN CORSO";
	
	if (realtime.fineround) mtext="FINE ROUND "+realtime.round;
	
	text+=", "+mtext;
	
	text+=", "+realtime.result;
...
function updRealtimeResult(data){
	...
	var testo="<font style='color: blue'>"+data.text+"</font>";
...
function getMedaglieGara($m)
	...
	console.log("refreshiscritti:",jcurrentgara);
  ...
  function setResult(obj)
	 {
	  ...
	  $('#popResult #radio_round1').prop('checked', true);
