	 ...
	 $(document).on("pageinit","#gara",function() {
		 $.ajaxSetup({ cache: false });
		 
	$("#dialogCategorie").popup();
	...
	$('.spancat').bind('click',function() {
		
		var lv=$("ul#ulcategorie");
		   lv.listview();
		   lv.listview("refresh");
		$("#dialogCategorie").popup();
		//$("#dialogCategorie").popup('open', {positionTo: '.spancat'});
		$("#dialogCategorie").popup('open');
		
	});
	
	$("#ulcategorie li").bind("click",function() {
	...
	$("#listabyprog li").bind("click",function() {
	   var id=$(this).attr("id");
	   ...
	   //alert(id);
	     ...
	     $("#navbar ul li").bind("click",function(){
			...
			$("#navbar ul li").removeClass("tabselected");
...
("ui-state-persist");
		 ...
		 lastcronaca=getCookie("lastcronaca");
		 
		 sdebug("lastcronaca from cookie: "+lastcronaca);
		 ...
		 $("#bubble").bind("click",function() {
			...
			notifycount=0;
			nonletti=0;
			$(".nonvisto").removeClass("nonvisto");
			$("#bubble").html("0").hide();
			
			$("a#tab_cronaca").trigger("click");
	  ...
	  function getNDT(data)
	{
	 ...
	 {
	 $("#navbar ul li:eq("+n+")").trigger("click");
	 return;
	 $( "#tabs" ).tabs( "option", "active", n );
	 
	 $(".tab-content div:eq("+n+")").removeClass("ui-screen-hidden");
	 ...
	 function refreshMe(forcerefresh,options){
	  var opt = { 
	   ...
	   oldupdatetext=$("#ulinfogara li span.left").html();
	   
	   $("#ulinfogara li span.left").html("Aggiornamento...");
    /*$.mobile.loading( 'show', {
	...
	/*   $.ajax({
	  ...
	  // $("#hdr").find("h1").html("Gara "+$gara.find("title").text()+" "+$gara.find("location").text()+" - "+$gara.find("data").text());
	   $("#hdr").find("#hgara a").html($gara.find("location").text()+" -     "+$gara.find("data").text());
...
$(document).on( 'taphold', 'ul#listampa li', tapholdHandler );
  ...
  */
	 ...
	 function matchesPerAtleta(atletaid,atletaname,datanasc)
	 {
	  ...
	  debug(selectedatletaid+" "+selectedatletaname);
	 ...
	 //alert(v);
	  ...
	  var url="updagent.php?action=edit&tag=match";
	    ...
	    var ln=$("ul#listampa li").length;
	   var ord=$("ul#listampa li#match_"+id).index();
	  ...
	  if ($.trim(result)!="") cronacatxt+=" per "+result;
	   ...
	   $("ul#listampa li").each(function(i) {
	      var id=$(this).attr("id").replace("match_","");
				...
				if (($.trim(gid)==$.trim(garaid)) && ($.trim(mid)==$.trim(matchid)) && ($.trim(atleta)==$.trim(atid))) 
{
	...
	// $.mobile.changePage("iscritti.php");
	...
	// alert(iscritti);
	 ...
	 $("#iscritti").append(htm);
	 $("#iscritti").listview();
	 $("#iscritti").listview("refresh");
		   
	 $("#infogaraiscritti").html(arr.length+" iscritti a questa gara");
