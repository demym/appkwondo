...
router.post('/addItem', function(req, res) {
	...
	req.session.installBuffer = installBuffer;
...
router.post('/addInstallation', function(req, res) {
	
	console.log("router post addInstallation");
	console.log(installBuffer.length);
	console.log(JSON.stringify(req.session));
	console.log("Storing "+req.session.installBuffer.length+" items as installation");
	...
	for (var i=0; i<req.session.installBuffer.length; i++)
	{
	 var it=req.session.installBuffer[i];
     addInst.items[i]=it;	 
