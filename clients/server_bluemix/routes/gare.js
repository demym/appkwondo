...
function readGareMongo(callback) {
	mongo.getfile(jfname,function(data){
		...
		gare=data;
		console.log("read gare from mongo "+jfname+" :   "+gare.rows.length);
        callback(); 
...
function readGareAltervista(callback){
 ...
 console.log("readGareAltervista")	
 var fname="gare.json";	
 request.get(altervistadataurl+fname, function (error, response, body) {
    if (!error && response.statusCode == 200) {
		...
		gare=data;
		console.log("read gare from altervista "+fname+" :   "+gare.rows.length);
		callback(gare);
...
function readGareJson(callback) {
   ...
   console.log("read gare from file "+fname+" :   "+gare.rows.length);
   callback(); 
   //console.log(data);
	});
...
fs.readFile(fname, "utf-8",function(err, data) {
   ...
   //console.log(data);
    ...
    parser.parseString(data, function (err, result) {
		...
		*/
		console.log("Loaded "+result.gare.gara.length+" gare");
		callback();
		//atleti=result;
...
router.get('/findall', function(req, res) {
	...
	console.log("called gare findall for societa "+societa);
	//readGareJson(function(){
	 ...
	 findAll(function(obj) {
		...
		for (var i=0; i< obj.rows.length; i++){
			...
			try {
			 ...
			 console.log("gara id "+garaid+" - ori: "+ori+" arg "+arg+" bro "+bro)
			// })
			} catch(e){
				console.log("error")
				row.ori=ori;
...
router.get("/fullgarabyid/:id",function(req,res){
	...
	console.log("called  fullGaraById for gara "+id+ " and societa "+societaid);
	...
	findById(id,function(obj) {
		...
		getmatchesbyprog(garaid,function(data){
			//console.log(data);
			console.log("--- got matches byprog");
			var mprog={
				rows: []
			}
			var rtmatches={
		...
		if (societaid!=""){ // se filtro su società
		  ...
		  console.log("filtro societa impostato")
			for (var i=0; i<data.rows.length; i++){
				...
				if (doc.societaid) socid=doc.societaid;
				  ...
				  console.log("---"+atl.societaid+" - societaid: "+societaid)
				  if (atl.societaid.trim()==societaid.trim()) 
				  {
					  console.log("pusho");
					  mprog.rows.push(row);
					  console.log("realtime match: "+rt);
			...
			getmatchesbyatleta(garaid,function(mbadata){
				console.log("got matchesbyatleta")
		  ...
		  console.log("filtro societa impostato")
			for (var i=0; i<mbadata.length; i++){
 			...
 			//console.log("matchid: "+doc.matchid+" - "+atl.cognome+" "+atl.nome+" - "+atl.societaid)
				  ...
				  console.log("---"+atl.id+" - societaid: "+societaid)
				  if (atl.societaid.trim()==societaid.trim()) 
				  {
					  console.log("pusho");
...
router.get('/findById/:id', function(req, res)  {
	...
	console.log("called gare findById for societa "+societa);
	var id=req.params.id;
	//res.send(risp);
	mongo.getfile("atleti.json",function(atleti){
...
router.post('/add', function(req, res) {
	...
	var mfname="data/matches_"+id+".json";
	var m={rows:[]};	
	...
	fs.writeFile(mfname, JSON.stringify(m), function (err) {
		  ...
		  console.log(dat.rows.length);
		  var fullUrl = req.protocol + '://' + req.get('host');
		  saveToAltervista(fullUrl+"/matches/findbygaraid/"+id,"matches_"+id+".json");
		  
		  
        });
		
	var cfname="data/cronaca_"+id+".json";
...
router.get('/resetcronaca/:garaid', function(req, res) {
	...
	var m={rows:[{time: jul, text: "Cronaca resettata"}]};
...
router.post('/update', function(req, res) {
	...
	console.log("trying to update gara id "+id);
	...
	for (var i=0; i<gare.rows.length; i++)
	{
     ...
     if (gid==id) {
	  ...
	  fs.writeFile('data/gare.json', JSON.stringify(gare), function (err) {
          if (err) return console.log(err);
          console.log('File gare.json updated on server');
		  var fullUrl = req.protocol + '://' + req.get('host');
		  saveToAltervista(fullUrl+"/gare/findall","gare.json");
...
function findByName(nome,callback)  {
    ...
    console.log('gare findByName=' + nome);
