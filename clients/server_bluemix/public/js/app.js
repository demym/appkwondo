...
//var rooturl="http://192.168.1.100:3000";
//var rooturl="http://192.168.1.105:3000";
...
var delmatchid="";
   ...
   */
	   ...
	   for (var x=0; x<realtimeArray.length; x++) {
			...
			for (var y=0; y<data.realtime.rows.length; y++) {
				...
				if (rkey==key) {
					...
					if (rt==true) {
						console.log("match "+rkey+" is in realtime, checking and adding in realtimearray");
						foundrt=true;
					}
			...
			if (!foundrt) {  //remove this match from realtime array
						...
						console.log("match "+key+" not anymore in realtime, deleting from realtimearray");
			}
   ...
   function addLastRealtimeData(data){
	   ...
	   console.log("addLastRealtimeData",data);
	   ...
	   for (var i=0; i<realtimeArray.length; i++) {
		   ...
		   if (rkey==key) {
			   console.log("found realtimeArray element to update");
	   ...
	   if (!found) console.log("match "+key+" not found in realtimeArray");
   }
   
   function checkRealtimeArray(data) {
	   
	   console.log("checkrealtimearray ",data);
	   
	   var key=data.garaid+"_"+data.id;
	  ...
	  */
	  ...
	  socket.on("chatmsg",function(data){
		 ...
		 // if (data.nickname==chatuser.nickname){
			  ...
			  if (data.nickname!=chatuser.nickname) {
				  if (pageid!="page_chat")  snotify({text: ntext});
			  }	  
			...
			if (jcurrentgara.id==data.garaid){
				refreshCurrentGara(function() {
				  ...
				  if (jGara.realtime.rows.length>0) {
					...
					$("#gara #btnChat").css("border","2px lightgreen solid");  
					  
				  } else $("#gara #btnChat").css("border","0px");
...
function updRealtimeResult(data){
	...
	colog("updateRealtimeResult "+mid+" - "+result);
	...
	/*
	$("#gara ul#listabyprog li").each(function(){
...
*/
	...
	/*
	$("#matchesatleta ul#listampa li").each(function(){
...
("ui-state-persist");
	...
	//gara.loadById(id,function(data) {
		...
		console.log("checking and syncing realtime matches")
...
function refreshNews(callback) {
 ...
 function renderCronaca($cronaca){
	        ...
	        /* $($cronaca.rows).each(function(i){
			 ...
			 console.log("non letti prima del <0: "+nonletti);
			 ...
			 console.log("cronaca non letti per questa gara: "+nonletti);
			 console.log("cronaca letti per questa gara",letti);
			 
			 console.log("cronaca non letti globale: "+crnonletti);
...
function getMedaglieGara($m)
	...
	function getMatchesCountForAtleta(atletaid){
		...
		$($allmatches.rows).each(function(i){
			...
			var id=$allmatches.rows[i].doc.atletaid;
	...
	conslog("refreshiscritti:",jcurrentgara);
...
function resetChat() {
	...
	gConfirm("Sei sicuro di voler resettare la chat per questa gara ? ?","Reset chat",function() {
		 $.ajax({
            //url: "http://ssodemyapp.mybluemix.net/schede/addScheda",
			url: rooturl+"/chat/reset/"+jcurrentgara.id,
            type: "GET",
		 data: {}})
        .done(function(data) {
...
function postChat(msg) {
	 ...
	 $.ajax({
            //url: "http://ssodemyapp.mybluemix.net/schede/addScheda",
			url: rooturl+"/chat/put/"+jcurrentgara.id,
            type: "POST",
            data: msg})
        .done(function(data) {
			console.log(data);
	...
	$.ajax({
				...
				url: rooturl+"/chat/get/"+jcurrentgara.id,
		     	success: function(data) {
					...
					if (!self) $.mobile.changePage("#page_chat");
